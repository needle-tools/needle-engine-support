...
FastHDR is a preprocessed, GPU-native environment map format built on **KTX2 supercompression** (UASTC HDR 4x4). Unlike traditional formats like EXR, FastHDR files stream directly to GPU memory — no heavy decompression on the main thread, no frame drops, no stalling.

The result: high-quality environment lighting that loads **10x faster** than EXR, uses **95% less GPU memory**, and requires **zero CPU processing**. No other web 3D framework offers this — it's a Needle Engine exclusive.

<video-embed src="https://www.youtube.com/watch?v=9v0C7Xa6TUE" limit_height />

## Why FastHDR?

Your environment lights up in **134 ms** instead of 1,200 ms.
Your GPU uses **12 MB** instead of 256 MB.
Your users download **7 MB** instead of 21 MB.
Your main thread does **nothing** — zero processing, zero frame drops.

And it looks **identical** to what you see in your editor.

Here's what that means in practice:

- **Budget phones stay smooth.** A $150 Android that chokes on a 256 MB EXR texture runs FastHDR without breaking a sweat — because it only needs 12 MB of VRAM
- **Pages load instantly.** Your 4K environment is ready in 134 ms. That's faster than most images on the web
- **No stutter, ever.** Traditional HDRIs block the main thread for decompression and mip generation. FastHDR skips all of that — the GPU handles everything directly
...
- **Full quality.** Pre-computed PMREM gives you physically accurate reflections and lighting — no shortcuts
...
Yes. Any `.exr` file you assign as environment lighting in Unity or Blender will be converted to FastHDR automatically when building for production.
