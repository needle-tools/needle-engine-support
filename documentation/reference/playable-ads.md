...
Configure these options inside `playableAds.build`:

```js
playableAds: {
    platforms: [{ platform: "applovin" }],
    build: {
        useRapier: false,
...
Omit an option to use automatic detection. Set `useRapier`, `usePostprocessing`, `useDraco`, or `useKTX2` to `true` or `false` to force the feature in or out. `noPeer: true` removes PeerJS networking.

Do not remove Draco or KTX2 while a packaged asset uses that decoder.
...
| --- | --- |
| `discord` | Deploy `dist/Discord/` to an HTTPS host. Add `@discord/embedded-app-sdk` to the application. |
| `youtube` | Upload `dist/YouTube/playable.zip` to the Playables Developer Portal. |
| `facebook-instant-games` | Upload `dist/FacebookInstantGames/playable.zip` to Instant Games Web Hosting. |
